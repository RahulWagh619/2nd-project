...
const formatDate = function (date, locale) {
  ...
  const daysPassedNext = daysPassed(new Date(), date);
  // console.log(daysPassedNext);
  if (daysPassedNext === 0) return 'Today';
  if (daysPassedNext === 1) return 'Yesterday';
  if (daysPassedNext <= 7) return `${daysPassedNext} days ago`;
...
const displayMovements = function (acc, sort = false) {
  ...
  const combinedDate = acc.movements.map(function (mov, i) {
    return { movement: mov, movementDate: acc.movementsDates[i] };
  });
  if (sort) {
    combinedDate.sort(function (a, b) {
      return a.movement - b.movement;
    });
  }


  combinedDate.forEach(function (obj, i) {
